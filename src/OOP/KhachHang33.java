...
public class KhachHang extends ConNguoi {
	...
	public KhachHang(String idKhachHang, String hoTen, String ngaySinh, String diaChi, String soDienThoai, int status,
...
public static KhachHang[] docFile() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (loai.equalsIgnoreCase("Thuong")) {
					KhachHang kh = new KhachHang(st[1], st[2], st[3], st[4], st[5], 1, Integer.parseInt(st[6]), 0);
					dsKH = Arrays.copyOf(dsKH, dsKH.length + 1);
					dsKH[dsKH.length - 1] = kh;
				} else if (loai.equalsIgnoreCase("VIP")) {
					KhachHang kh = new KhachHang(st[1], st[2], st[3], st[4], st[5], 2, 0, Double.parseDouble(st[6]));
					dsKH = Arrays.copyOf(dsKH, dsKH.length + 1);
					dsKH[dsKH.length - 1] = kh;
				}
			}
	...
	public static void ghiFile(KhachHang[] dsKH) {
		String fileName = "kh.txt";
   ...
   public static void xuatTatCa(KhachHang[] dsKH) {
		System.out.println("\n===== DANH SÁCH KHÁCH HÀNG THƯỜNG =====");
		xuatDanhSach(dsKH, 1);
		System.out.println("\n===== DANH SÁCH KHÁCH HÀNG VIP =====");
		xuatDanhSach(dsKH, 2);
	}
	public static void xuatDanhSach(KhachHang[] dsKH, int status) {
		if (status == 1) {
	...
	public static KhachHang[] themKhachHang(KhachHang[] dsKH, int status) {
		...
		if (status == 1) {
			...
			int slm = sc.nextInt();
			KhachHang kh = new KhachHang(id, ten, ns, dc, sdt, 1, slm, 0);
			dsKH = Arrays.copyOf(dsKH, dsKH.length + 1);
			dsKH[dsKH.length - 1] = kh;
		} else {
			System.out.print("Nhập tổng chi tiêu: ");
			double tong = sc.nextDouble();
			KhachHang kh = new KhachHang(id, ten, ns, dc, sdt, 2, 0, tong);
			dsKH = Arrays.copyOf(dsKH, dsKH.length + 1);
			dsKH[dsKH.length - 1] = kh;
		}
	...
	public static void timKiem(KhachHang[] dsKH, String id) {
		for (KhachHang kh : dsKH) {
	...
	public static void suaKhachHang(KhachHang[] dsKH, int status) {
		Scanner sc = new Scanner(System.in);
	...
	public static KhachHang[] xoaKhachHang(KhachHang[] dsKH) {
		Scanner sc = new Scanner(System.in);
		System.out.print("Nhập ID KH cần xóa: ");
		String id = sc.nextLine();
		boolean found = false;
		KhachHang[] moi = new KhachHang[0];
...
public static void main(String[] args) {
    Scanner sc = new Scanner(System.in);
    KhachHang[] dsKH = docFile();

    xuatTatCa(dsKH);
    System.out.print("\nChọn loại khách hàng để thao tác (1 - Thường, 2 - VIP): ");
    ...
    do {
        System.out.println("\n=== MENU KHÁCH HÀNG ===");
        System.out.println("1. Xuất danh sách loại đã chọn");
        ...
        switch (chon) {
            case 1 -> xuatDanhSach(dsKH, status);
            case 2 -> dsKH = themKhachHang(dsKH, status);
            case 3 -> suaKhachHang(dsKH, status);
